...
class NavigationState(
    ...
    /**
     * Returns true if the current route is a top-level destination (bottom nav item).
     * Used to show/hide the bottom navigation bar.
     */
    val isOnTopLevelDestination: Boolean
        get() {
            val currentStack = backStacks[topLevelRoute] ?: return false
            return currentStack.size == 1 && currentStack.lastOrNull() in backStacks.keys
        }
}
...
val serializersConfig = SavedStateConfiguration {
    serializersModule = SerializersModule {
        polymorphic(NavKey::class) {
            ...
            subclass(Route.SettingsDetail::class, Route.SettingsDetail.serializer())
        }
    }
}
